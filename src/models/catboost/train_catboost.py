# -*- coding: utf-8 -*-
"""
CatBoost ile ramp-up oncul matrislerinden delta_ws ve delta_t tahmini.

EKSIK VERI: DOLDURULMAZ. CatBoost NaN'i dogrudan isler (varsayilan
nan_mode="Min": eksik degerler bolunmede ayri bir yon olarak ele alinir).
Projenin "sentetik doldurma yapilmaz" kuralina birebir uyar.

Degerlendirme: model/_ortak/degerlendirme.py'deki ortak cerceve
(blok-farkinda 5 kat x 5 tekrar CV + temel cizgi) - CNN-LSTM ve XGBoost
ile BIREBIR ayni bolmeler.
"""
...
# Kucuk veri icin sig agaclar + guclu duzenlilestirme
PARAMS = dict(
    depth=2,
    learning_rate=0.03,
    l2_leaf_reg=10.0,
...
for rep, f, inner_tr, inner_va, te in ev.fold_iter(groups):
    for ti in range(2):
        ...
        model.fit(Pool(X[inner_tr], y[inner_tr, ti]),
                  eval_set=Pool(X[inner_va], y[inner_va, ti]),
                  early_stopping_rounds=50, verbose=False)
        best_iters[ti].append(model.get_best_iteration())
...
for ti, tname in enumerate(ev.TARGETS):
    p = dict(PARAMS)
    p["iterations"] = max(int(np.median(best_iters[ti])), 10)
    m = CatBoostRegressor(**p)
    m.fit(Pool(X, y[:, ti]), verbose=False)
    train_pred[:, ti] = m.predict(X)
    imp = pd.DataFrame({"feature": names, "gain": m.get_feature_importance()})
...
for ti in range(2):
    ...
    p["iterations"] = max(int(np.median(best_iters[ti])), 10)
    m = CatBoostRegressor(**p)
    m.fit(Pool(X[train_mask], y[train_mask, ti]), verbose=False)
...
with open(OUT_DIR / "split_info.json", "w", encoding="utf-8") as f:
    json.dump({"n_olay": int(N), "n_blok": int(len(np.unique(groups))),
               ...
               "eksik_veri": "DOLDURULMADI - CatBoost NaN'i dogrudan isler",
               "medyan_en_iyi_iterasyon": {k: int(np.median(v)) for k, v in best_iters.items()},
