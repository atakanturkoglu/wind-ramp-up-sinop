# -*- coding: utf-8 -*-
"""
Standart LSTM (CNN katmani YOK) ile delta_ws ve delta_t tahmini.

CNN-LSTM ile tek farki: girdi dogrudan LSTM'e verilir, oncesinde Conv1d
ile kanal indirgeme yapilmaz. Parametre sayisi benzer tutulmustur
(~1.070 vs CNN-LSTM'in ~1.180), boylece "CNN katmani bir sey katiyor mu"
sorusu tek degiskenli olarak yanitlanabilir.
...
n_par = sum(p.numel() for p in PlainLSTM().parameters())
...
# Ezberleme miktarini gormek icin OOF ile karsilastirilir.
...
final_model = PlainLSTM().to(DEVICE)
...
# her olay ACIKCA ya train ya test, iki kutu degil). Metrikler CV'den gelir.
...
split_model = PlainLSTM().to(DEVICE)
...
with open(OUT_DIR / "split_info.json", "w", encoding="utf-8") as f:
    json.dump({"n_olay": int(N), "n_blok": int(len(np.unique(groups))),
               ...
               "mimari": {"hidden": HIDDEN, "dropout": DROPOUT,
                          "weight_decay": WEIGHT_DECAY, "cnn": False}},
...
for ti, tname in enumerate(["delta_ws", "delta_t"]):
    ...
    ax.set_title(f"LSTM Feature Importance (Permutation) — {target_labels[tname]}\n"
